...
- [x] **AI Lab 后台**：⚙ 齿轮图标入口 + `/admin` 管理面板
  - 左侧三场景沙盒控制台：快速录入 / 采访式 / 增量更新（均可交互操作）
  - Prompt 模板管理：云端保存/加载/删除 + 本地 JSON 导入导出
  - 4 个 Prompt 实时编辑器 + 模型选择 + Temperature 滑块
  - 采访收敛条件配置 + AI 处理日志
- [x] **Admin Basic Auth**：`/admin` 和 `/api/admin/*` HTTP Basic Auth 保护
- [x] **超长文本保护**：前端 2000 字限制 + API 3000 字符校验(413) + AI 30s 超时
